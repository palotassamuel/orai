...
public class BookWindow extends Window{
		...
		saveButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				try {
					...
					Notification.show("Sikeres könyv hozzáadás/módosítás");
					bookView.refreshTable();
					
				} catch (Exception e) {
					Notification.show("Hiba történt a mentés/módosítás folyamán");
				}
	...
	private Component createFormLayout(BeanFieldGroup<Book> bookBeanField, boolean isReadonly) {
		...
		TextField titleFild=bookBeanField.buildAndBind("Könyv címe","title",TextField.class);
		titleFild.setNullRepresentation("");
		titleFild.setReadOnly(isReadonly);
		formLayout.addComponent(titleFild);
